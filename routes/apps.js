...
if (setting.template.apps) {
  ...
  app.post("/source", (req, res) => {
    upload_file(req, res, function (err) {
      ...
      if (req.files) {
        ...
        res
          .status(400)
          .send({ error: "No file was uploaded with the request." });
        return;
      }
      var app_post = {
        zip_url: zipFileName,
  ...
  //chỗ định tuyến
  app.get("/app/:id", appUtils.csrfProtection(), function (req, res) {
    getFlow(req.params.id, null, req, res);
  });
  app.get(
    "/app/:id/in/:collection",
    appUtils.csrfProtection(),
    function (req, res) {
      getFlow(req.params.id, req.params.collection, req, res);
    }
  );

  function getFlow(id, collection, req, res) {
    apper
      .get(id) //lấy giá trị từ db gán vào biến app
      .then(function (app) {
        //
        ...
        if (req.cookies.rateID) {
          ...
          if (app.rating && !app.rating.hasOwnProperty("count")) {
            ...
            ratingPromise = ratings
              .getUserRating(id, req.cookies.rateID)
        ...
        if (!app.flow) {
          ...
          try {
            ...
            app.nodeTypes.sort(function (a, b) {
              if (
                a.type in npmNodes.CORE_NODES &&
                !(b.type in npmNodes.CORE_NODES)
              ) {
                return -1;
              }
              if (
              ...
              mark(data, {}, function (err, content) {
                ...
                ratingPromise
                  ...
                  .then(function (collectionSiblings) {
                    ...
                    res.send(
                      //đây là nơi render giá trị
                      mustache.render(templates.app, app, templates.partials)
                    );
                  });
              });
            }
  ...
  app.get("/update/app/source/:id", verifyOwner, function (req, res) {
    apper
      ...
      .then((app) => {
        ...
        app.imgGuidelineUrl = imgUrl;
        res.send(
          mustache.render(templates.updateSourceApp, app, templates.partials)
        );
      })
      .catch(function (err) {
        // TODO: better error logging without the full stack trace
  ...
  app.post("/app/:id/tags", verifyOwner, function (req, res) {
    ...
    apper
      .updateTags(req.params.id, req.body.tags)
      .then(function () {
        res.status(200).end();
      })
      .catch(function (err) {
  ...
  app.post("/app/:id/rate", appUtils.csrfProtection(), function (req, res) {
    ...
    if (req.cookies.rateID) {
      ratings
        .rateThing(id, req.cookies.rateID, Number(req.body.rating))
        .then(function () {
          res.writeHead(303, {
            Location: "/app/" + id,
          });
  ...
  app.get("/add/app/source", function (req, res) {
    ...
    context.display = setting.template;
    res.send(
      mustache.render(templates.addSourceApp, context, templates.partials)
    );
  });
  module.exports = app;
}
