...
function getExtension(id, projection) {
...
async function putExtension(id, data) {
    try {
...
function refreshExtension(id) {
    return new Promise((resolve, reject) => {
        getExtension(id, { etag: 1, tags: 1, added_at: 1 }).then(function (app) {
...
function createExtension(accessToken, extension, tags) {
    return new Promise((resolve, reject) => {
...
async function updateExtensionSource(id, data) {
    ...
    if (data.version) newVersion.versions = {
        version: data.version,
        last_update: new Date().toLocaleDateString()
    }
...
function removeExtension(id) {
    return new Promise((resolve, reject) => {
        getExtension(id).then(function (app) {
...
function getExtensionsForUser(userId) {
    return getExtensions({ 'owner.login': userId });
}
function getExtensionsForTag(tag) {
    return getExtensions({ tags: tag });
}
...
function updateTags(id, tags) {
    tags = tags || [];
    return new Promise((resolve, reject) => {
        getExtension(id, { tags: 1, description: 1, 'files.README-md': 1, 'owner.login': 1 }).then(function (app) {
...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        if (path.extname(file.originalname) === '.zip') {
            if (!fs.existsSync(path.join(url_zip, req.body.version + ''))) {
                fs.mkdirSync(path.join(url_zip, req.body.version + ''), { recursive: true });
            }
            cb(null, path.join(url_zip, req.body.version + ''));
        } else {
            cb(null, url_zip);
        }
    },
...
module.exports = {
    ...
    getForUser: getExtensionsForUser,
    getUser: getUser,
    create: createExtension,
    getAllTags: getAllTags,
    getForTag: getExtensionsForTag,
    storage, multi_upload,
    putSource: putExtension,
